...
int main() {
    ...
    for (int i = 0; i < MAX_OP; i++) {
        switch (rand() % 3) {
            case 0: p->next = getNode_A(); break;
            case 1: p->next = getNode_B(); break;
            case 2: p->next = getNode_C(); break;
            default: break;
        }
        p = p->next;
    }
    for (p = head.next; p; p = p->next) {
        switch (p->type) {
            case type_A: output_A(p); break;
            case type_B: output_B(p); break;
            case type_C: output_C(p); break;
            default: break;
        }
    }
    return 0;
